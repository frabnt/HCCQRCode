...
import main.java.com.google.zxing.hccqrcode.decoder.Mode;
...
public final class Encoder {
	 ...
	 */
	  ...
	  private static int calculateMaskPenalty(ByteVectorMatrix matrix) {
	   ...
	   *   or configuration
	   */
	  public static HCCQRcode encode(String content, ErrorCorrectionLevel ecLevel) throws WriterException {
	    return encode(content, ecLevel, null);
	  }

	  public static HCCQRcode encode(String content,
	    ...
	    HCCQRcode qrCode = new HCCQRcode();
	    ...
	    ByteVectorMatrix matrix = new ByteVectorMatrix(dimension, dimension);
	  ...
	  private static int chooseMaskPattern(BitArray bits,
	    ...
	    for (int maskPattern = 0; maskPattern < HCCQRcode.NUM_MASK_PATTERNS; maskPattern++) {
	    ...
	    for (int i = 0; i < maxNumDataBytes; ++i) {
	      ...
	      for (BlockPair block : blocks) {
	        ...
	        if (i < dataBytes.length) {
	          //System.out.println("DataBlock="+dataBytes[i]);
	          result.appendBits(dataBytes[i], 8);
	        }
	      }
	    }
	    //System.out.println("Ho incotrato "+(count2-1)+" volte");
	    // Then, place error correction blocks.
	    //System.out.println("/* CORRECTION BLOCKS */");
	    for (int i = 0; i < maxNumEcBytes; ++i) {
	      for (BlockPair block : blocks) {
	        byte[] ecBytes = block.getErrorCorrectionBytes();
	        if (i < ecBytes.length) {
	          //System.out.println("ECBytes="+ecBytes[i]);
